...
import {
  getGalEffectsAssetStream,
  parseGalEffectsHost,
  resolveGalEffectsAssetKey,
...
/**
 * Public media proxy for Gal Toolkit Assets under private bucket `gal-toolkit-max`.
 * `<img>` / `<video>` cannot send Bearer — keep catalog JSON gated, Assets
 * streamable (same pattern as premiere-gal showcase-media).
 */
const CORS_HEADERS = {
...
async function serve(req: NextRequest, context: RouteContext) {
  ...
  const objectKey = resolveGalEffectsAssetKey(segments ?? [], host);
  if (!objectKey) {
  ...
  try {
    ...
    const obj = await getGalEffectsAssetStream(objectKey, range, host);
    if (!obj) {
      return NextResponse.json(
        { error: "Not found" },
        { status: 404, headers: CORS_HEADERS },
    ...
    console.error("[cep/gal/effects/media]", err);
    return NextResponse.json(
