...
/**
 * Owner / ops: adjust `user_generation_credits` (SSOT) + mirror `users.extra_generations_count`.
 *
...
export async function POST(req: NextRequest) {
    ...
    if (!hasExtra && !hasSub) {
        return NextResponse.json(
            {
                error:
                    "Provide setExtraBalance and/or subscriptionAdjustment",
            },
            { status: 400 },
        );
    }
    ...
    const result = await adminApplyCreditChanges({
        userId,
        setExtraBalance: hasExtra
            ? Number(body.setExtraBalance)
            : undefined,
