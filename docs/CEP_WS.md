...
Device limit default is **3** (`CEP_DEVICE_LIMIT`) **per CEP client** (`gal-cep` and `spunkram-cep` are independent). Re-login of a seat already in the list (same MAC+client, or same OS username at the limit) completes normally. A new occupant gets `device_limit` + devices; panel calls `POST /api/cep/auth/replace-device`.
